...
class SnipCommandEdit(QtWidgets.QPlainTextEdit):
    ...
    def _check_slash_command(self):
        ...
        if not treffer:
            # Hilfreicher Hinweis statt leerem Popup
            if not snips:
        ...
        # Popup unter dem Feld positionieren
    ...
    def _eintrag_gewaehlt(self, item):
        if not (item.flags() & QtCore.Qt.ItemIsEnabled):
            return  # deaktivierter Hinweis-Eintrag
    ...
    def keyPressEvent(self, event):
        if self._popup.isVisible():
            if event.key() == QtCore.Qt.Key_Down:
                row = min(self._popup.currentRow() + 1,
                          self._popup.count() - 1)
    ...
    def focusOutEvent(self, event):
        # Verzögerung: Mausklick auf Popup-Item / Scrollbar muss noch
        # verarbeitet werden, bevor wir entscheiden ob das Popup bleibt.
        if self._popup.isVisible():
            QtCore.QTimer.singleShot(250, self._verberge_popup)
        super().focusOutEvent(event)

    def _ist_popup_widget(self, widget) -> bool:
        """Gibt True zurück wenn widget zum Popup-Bereich gehört.

        Prüft nicht nur den Popup selbst, sondern auch:
          • Alle Kind-Widgets (Scrollbar, Viewport, …)
          • Den Header
        Damit schließt das Popup NICHT wenn die Scrollbar angefasst wird.
        """
        if widget is None:
            return True   # kein Fokus = Klick auf nativen Scrollbar-Bereich
        if widget is self or widget is self._popup or widget is self._popup_header:
            return True
        # Prüfe ob widget ein Nachkomme von _popup ist (z.B. QScrollBar)
        if self._popup.isAncestorOf(widget):
            return True
        return False

    def _verberge_popup(self):
        """Versteckt Popup nur wenn der Fokus wirklich woanders ist."""
...
class _BlauBanner(QtWidgets.QFrame):
    ...
    def paintEvent(self, event):
        ...
        col.setAlpha(60)   # halbtransparent → passt zu hell und dunkel
